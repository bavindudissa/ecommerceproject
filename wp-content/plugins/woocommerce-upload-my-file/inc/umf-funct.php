...
function umf_my_orders_uploads_column($order)
{
    ...
    if(check_for_files(umf_get_order_id($order))=='upload') {
	    echo '<a href="'.$order->get_view_order_url().'">'.__( 'Upload', 'woocommerce-umf' ).'</a>';
	} elseif(check_for_files(umf_get_order_id($order))=='blank') { echo '-'; }
...
function woocommerce_umf_get_product_meta( $product, $field_name ) {
    ...
    $product_id = umf_get_product_data($product, 'id');

    return get_post_meta($product_id, '_'.$field_name, true);
...
function get_max_upload_count($order,$order_item=0) {
	$max_upload_count=0;
	//product specifiek
	if( (( is_array( get_option( 'woocommerce_umf_status' ) ) && in_array( umf_get_order_data($order, 'status'), get_option( 'woocommerce_umf_status' ) ) ) ) ||  umf_get_order_data($order, 'status') == get_option( 'woocommerce_umf_status' ) ) {
	   	if($order_item!=0) {
	...
	if( isset( $_FILES ) && isset($_POST['upload']) && $_POST['upload']) {
		...
		foreach( $_FILES as $key => $umf_file ) {
			...
			if($_FILES[$key]["size"]< ($filesize*1024*1024))
				{$sizeallow=true; } else {$sizeallow=false; $upload_error.= '<li">' . sprintf( __( 'The file "%s" is too big.', 'woocommerce-umf'), $umf_file['name'] ) . '</li>';}
	...
	echo '<form enctype="multipart/form-data" action="" method="POST" class=woo-umf-form>';
		...
		foreach ( $order->get_items() as $order_item ) {
			...
			if($max_upload_count!=0){
				...
				if(!empty($variation_data)) {$variation='<span class=woo-umf-variations>'.$variation_data.'</span>';} else {$variation='';}
			...
			while ($i <= $max_upload_count) {
				...
				if($name=="") {
					...
					echo '<input type="hidden" value="'.strip_tags($order_item['name'].' - '.$variation_data).'" name="uploaded_product_name['.$j.']">';
					$upload=true;
				} else {
	...
	if($count>0) {
		if(check_for_files(umf_get_order_id($order))=='upload') {
			...
			if ( version_compare( WOOCOMMERCE_VERSION, "2.1" ) <= 0 ) {
				echo '<p><a href="'. esc_url( add_query_arg('order', umf_get_order_id($order),get_permalink(woocommerce_get_page_id('view_order'))) ).'" ><span>'._n( 'Login to upload your file and attach it to your order.', 'Login to upload your files and attach them to your order.', $count, 'woocommerce-umf' ).'</span></a>';
			} else {
...
/*
     ...
     */
     ...
     function umf_get_order_id($order)
     {

        return umf_get_order_data($order, 'id');
     ...
     */
     ...
     function umf_get_order_data($order, $data)
     {
        if( version_compare( WC_VERSION, '3.0', '<' ) ) {
     ...
     /*
     * Get product data by Product object
     *
     * Used for backward compatibility for WC < 3.0
     ...
     */
     ...
     function umf_get_product_data($product, $data)
     {
        if( version_compare( WC_VERSION, '3.0', '<' ) ) {
     ...
     function woo_umf_get_variation_info($item) {
             ...
             if (version_compare(WC_VERSION, '3.1', '<')) {
                 ...
                 if (is_array($attributes) && !empty($attributes)) {
                     foreach ($attributes as $attribute) {
                         $variation .= strip_tags(trim($attribute->display_key).' : '.trim($attribute->display_value).', ');
                     }

                     $variation = substr($variation, 0, -2);
                 } else {
                     $variation = null; // Variatin info in product title if attribute amount < 2 (since WC 3.0?)
                 }
             }
