...
function eufdc_unique_identity() {
	...
	global $easy_ufdc_page, $easy_ufdc_req, $easy_ufdc_error, $wufdc_dir, $wufdc_dir_url, $ufdc_premium_link, $ufdc_custom,
		   ...
		   $eufdc_default_parent_post_id, $wc_ufdc_upload_dir, $eufdc_product_round, $easy_ufdc_success, $easy_ufdc_multiple, $is_view_order, $eufdc_input_text_label, $eufdc_items_attachments, $eufdc_localize_arr;
...
if ( is_dir( $wc_ufdc_upload_dir ) ) {
	...
	$eufdc_items_attachments = get_option( 'eufdc_items_attachments' );

	$eufdc_items_attachments = ( 'product' == $easy_ufdc_page && 1 == $eufdc_items_attachments );
...
if ( ! is_admin() ) {
	...
	switch ( $easy_ufdc_page ) {
		...
		case 'checkout_above':
			if ( function_exists( 'eufdc_open_form' ) ) {
				add_action( 'woocommerce_before_checkout_form', 'eufdc_open_form' );
			}
			break;
		case 'thank_you':
			if ( function_exists( 'eufdc_open_form' ) ) {
